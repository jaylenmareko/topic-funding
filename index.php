...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .strip-avatar-label { font-size: 13px; font-weight: 600; color: #fff; letter-spacing: 0.1px; }
        ...
        .strip-creator-card-bio { font-size: 11px; color: #777; margin-top: 1px; line-height: 1.35; }
        ...
        .strip-funded-label { font-size: 13px; font-weight: 700; letter-spacing: 0.4px; text-transform: uppercase; color: #F59E0B; margin-bottom: 10px; padding-bottom: 6px; border-bottom: 1px solid rgba(245,158,11,0.25); }
...
<body>
    ...
    (function () {
        ...
        function renderFundedTopics(creatorId) {
            const all = CREATOR_FUNDED[creatorId] || [];
            if (all.length === 0) { stripFundedTopics.classList.remove('visible'); return; }
            const funded = all.filter(t => t.status === 'funded');
            const queued = all.filter(t => t.status === 'queued').sort((a, b) => new Date(a.funded_at || 0) - new Date(b.funded_at || 0));
            const onHold = all.filter(t => t.status === 'on_hold');
