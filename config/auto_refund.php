...
try {
    ...
    if (!empty($results)) {
        ...
        $admin_message .= "Topics processed: {$total_topics}\n";
    ...
    if ($_SERVER['HTTP_HOST'] === 'localhost' || strpos($_SERVER['HTTP_HOST'], '127.0.0.1') !== false) {
        error_log("ADMIN CRON ERROR: " . $error_message);
    } else {
        mail('admin@topiclaunch.com', 'Auto-Refund CRON FAILED - TopicLaunch', $error_message, 'From: system@topiclaunch.com');
    }
}
